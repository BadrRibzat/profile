...
const ResumeGenerator: React.FC = () => {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        setLoadError(t("errors.unavailable", { locale: locale.toUpperCase() }));
        // Fallback to English
        try {
  ...
  const getLocalizedTitle = () => {
    const titles = {
      en: t("titles.professional"),
      de: t("titles.german"),
      fr: t("titles.french"),
      es: t("titles.spanish"),
      ar: t("titles.arabic"),
      ja: t("titles.japanese"),
    };
    ...
    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center space-x-2">
          ...
          <span className="text-sm text-gray-600 dark:text-gray-400">{t("subtitle")}</span>
        </div>
      </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-4 text-sm">
            <span className="flex items-center gap-1">
              ...
              {t("features.atsOptimized")}
            </span>
            <span className="flex items-center gap-1">
              <Briefcase className="w-4 h-4 text-green-500" />
              {t("features.visaReady")}
            </span>
          </div>
        </div>
        ...
        <div className="flex justify-center mt-4">
          {isClient ? (
                ...
                >
                  {loading ? (
                    ...
                    <>
                      ...
                      <span>{t("buttons.error")}</span>
                    </>
                  ) : (
                    <>
                      <Download className="w-5 h-5" />
                      <span>{t("buttons.download", { locale: locale?.toUpperCase() })}</span>
                    </>
                  )}
                </motion.button>
              )}
            ...
            <div className="w-full max-w-sm flex items-center justify-center gap-2 py-3 px-6 rounded-lg bg-gray-200 dark:bg-gray-700">
              <Loader className="w-5 h-5 animate-spin" />
              <span className="text-gray-600 dark:text-gray-300">{t("buttons.loadingLink")}</span>
            </div>
          )}
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
        <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
          <h4 className="font-medium text-blue-800 dark:text-blue-300 mb-1">{t("meta.style")}</h4>
          <p className="text-blue-600 dark:text-blue-400">{t(`meta.styleValues.${locale}`)}</p>
        </div>
        <div className="p-3 bg-green-50 dark:bg-green-900/20 rounded-lg">
          <h4 className="font-medium text-green-800 dark:text-green-300 mb-1">{t("meta.optimization")}</h4>
          <p className="text-green-600 dark:text-green-400">{t("meta.optimizationDesc")}</p>
        </div>
        <div className="p-3 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
          <h4 className="font-medium text-purple-800 dark:text-purple-300 mb-1">{t("meta.purpose")}</h4>
          <p className="text-purple-600 dark:text-purple-400">{t("meta.purposeDesc")}</p>
        </div>
      </div>
    </div>
  );
